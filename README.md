...
## 정책 및 시스템 고도화 방향과의 연관성

국세청은 홈택스의 안정적 운영과 세무플랫폼 연계를 위해 스크래핑 대응, 오픈API 시스템 설계, 납세자 동의 기반 국세자료 제공 체계 등을 검토하고 있다.

본 프로젝트는 이러한 방향성과 연결하여, 사업자등록 단계에서 이미 보유한 업종코드·업태·종목 정보를 장부 작성 단계의 계정과목 추천에 활용하는 개선안을 제안한다.

이는 무분별한 스크래핑이 아니라, 공식 API 또는 홈택스 내부 데이터 연계를 통해 개인사업자의 장부 작성 편의성을 높이는 서비스 시나리오로 볼 수 있다.
